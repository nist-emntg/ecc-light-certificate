...
/**
 * @brief Compute the point d and Q (where d is the secret and Q is d.G)
 *
 * G is the generator
 *
 * @param secret[NUMWORDS] store d, a NUMBYTES bytes random number (generated by the function and must be passed to ecc_ecdh_from_network() later on)
 * @param point[2 * NUMBYTES] store the new point (Q)
 *
 * @return 0 on success. Always succeed.
 */
...
/**
 * @brief Compute the point d.Q' (where Q' is the other participant's public key)
 *
 * @param secret[NUMWORDS] NUMBYTES bytes secret computed by ecc_ecdh_from_host()
 * @param point[2*NUMBYTES] point that was transmitted by the other party (Q')
 * @param shared_secret[2*NUMBYTES] store the shared secret (d.Q')
 *
 * @return 0 on success. Always succeed.
 */
 ...
 *
 * @param shared_secret[2*NUMBYTES] point that contains the shared secret (d.Q')
