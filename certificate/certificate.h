 ...
 * @{ */
#define CERT_SIZE (4*2*arrayLength + SHA256_DIGEST_LENGTH + 2 * 36 + 4*arrayLength)
#define PUB_CERT_SIZE (4*2*arrayLength + SHA256_DIGEST_LENGTH + 2 * 36)
#define SIG_LEN 72
#define POINT_SIZE (32 + 32)
/**  @} */
...
{
	/** ECC pubkey (1/2) */
	uint32_t pub_x[arrayLength];
	/** ECC pubkey (2/2) */
	uint32_t pub_y[arrayLength];
 ...
 *
 * g is (ecc_g_point_x, ecc_g_point_y)
 *
 * @param secret[32] store a 32 bytes random integer (generated by the function and must be passed to ecc_ecdh_from_network() later on)
 * @param point[POINT_SIZE] store the new point (g^k)
 *
 * @return 0 on success. Always succeed.
 */
 ...
 *
 * @param secret[32] 32 bytes secret computed by ecc_ecdh_from_host()
 * @param point[POINT_SIZE] point that was transmitted by the other party (contains g^j)
 * @param shared_secret[POINT_SIZE] store the shared secret (g^(k+j))
 *
 * @return 0 on success. Always succeed.
 */
 ...
 *
 * @param shared_secret[POINT_SIZE] point that contains the shared secret (g^(k+j))
